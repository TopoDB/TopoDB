...
test("launch.js resolves TOPODB_PLUGIN_DATA, logs it, and serves a degraded MCP server when bootstrap fails", async () => {
  const tmp = mkdtempSync(path.join(tmpdir(), "codex-launch-"));
  ...
  const child = spawn(process.execPath, [LAUNCH], { env: { ...process.env, TOPODB_PLUGIN_DATA: dataDir, CLAUDE_PROJECT_DIR: HERE }, stdio: ["pipe", "pipe", "pipe"] });
  let out = "", err = "";
...
test("when the daemon socket never binds, launch.js still serves real MCP tools over stdio", async () => {
  const tmp = mkdtempSync(path.join(tmpdir(), "codex-stdio-fg-"));
