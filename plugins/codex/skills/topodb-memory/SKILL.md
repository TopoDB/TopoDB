...
project** plus a **`shared`** layer that crosses projects. Codex injects
recent project memories as context when a session starts — and again after
compaction — but they are the headlines, not the whole store.
...
pass a scope to read. Recent project memories are auto-injected when a session
starts — search for *specifics* rather than re-fetching what is already in
context.
