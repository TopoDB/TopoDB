...
mod graph;
pub use graph::{
    graph_edge, graph_node, node_superseded, node_title, to_canonical_json, GraphEdge, GraphNode,
    GraphSnapshot, GraphTruncation, GraphView, GRAPH_DEFAULT_LIMIT, GRAPH_MERMAID_INLINE_MAX_NODES,
    GRAPH_SNAPSHOT_VERSION, GRAPH_TITLE_MAX_CHARS,
};
...
pub const MEMORY_KIND_PROP: &str = "kind";
...
/// Enum-validates a caller-supplied memory kind. Exact-match, lowercase
/// only — a taxonomy is a vocabulary, not a suggestion box.
pub fn validate_memory_kind(kind: &str) -> Result<(), String> {
