...
| `memory_health` | `stale_older_than_days` (≥ 0, default 30), `scope?`, `scopes?` | One call that runs all three maintenance scans and returns a consolidated summary: `duplicate_pairs` vs `supersession_pairs`, `orphan_count`, `stale_count`, a `needs_attention` flag, and sample rows. The session-start "what needs tidying?" read. When the embedder is Ready, the duplicate/supersession split is vector-sourced (contradictions distinguished by negation cues). When the embedder is Failed or Downloading (degraded), all pairs count as `duplicate` (text-mode Jaccard, no contradiction detection), `supersession_pairs` forced to 0, and `needs_attention` forced true (even if counts are low). Degraded status is reported in `degraded`/`degraded_reason` fields. When the embedder is deliberately off, `duplicate_pairs`/`supersession_pairs` are 0 and `degraded` is false. |
