...
`tools/list` reports exactly 33 tools: `db_info`, `onboarding_pointer`, 16 read tools
(`get_changes` included), and 15 write tools.
...
|---|---|---|
| `db_info` | — | Report the open database's path, current op-log sequence number, the default WRITE scope applied to a create/link call that omits `scope`, the default READ scope set applied to a read call that omits both `scope`/`scopes`, an `embeddings: { model, status }` field (`status`: `off`/`downloading`/`ready`/`failed`) reporting the local embedding subsystem's state, and a `warehouse: { enabled, path?, mirrored_seq, spool_backlog }` field (when configured) reporting the mirror sync state and ingest queue depth. Call this first to confirm the server is wired to the expected database and read set, to obtain `current_seq` as the anchor for `get_changes`, and to know whether `search_memories` currently has a vector leg available and whether the warehouse is synced. |
| `onboarding_pointer` | — | Canonical onboarding pointer text + version (`topodb_onboarding::pointer_block()` / `ONBOARDING_VERSION`), for code clients (Claude Code plugin, pi) that reach TopoDB only over MCP and can't run the `topodb` CLI. No db access. |
...
| `search_memories` | `query` (required), `k` (integer, default 10), `kinds` (string[], optional), `recency_weight` (0-1, default 0.3), `recency_half_life_days` (default 30), `fuzzy` (bool, default true), `graph_boost` (bool, default true), `created_after`/`created_before` (ISO date or UTC datetime, optional), `temporal_rewrite` (bool, default true), `scope?`, `scopes?` | Hybrid recall (`Db::recall`) fusing up to three legs with Reciprocal Rank Fusion (k=60): a BM25 **text** leg over indexed content and entity names (camelCase-split, Snowball-stemmed, with miss-only fuzzy/prefix fallback at a 0.6× discount unless `fuzzy: false`, and learned synonyms — `add_synonym` — expanded into the query automatically); a cosine **vector** leg using an automatically-computed embedding of `query` whenever the server's embedder status is `ready` (silently omitted otherwise — see `db_info`); and, when `graph_boost` is true (default), a **graph** leg that takes the top 5 preliminary text+vector hits as seeds, pulls in their 1-hop neighbors at half weight, and folds them into the same fusion. Recency weighting is applied once, after fusion: each hit's fused score is multiplied by `(1-w) + w·2^(-age/half_life)`, so fresher memories outrank stale ones at equal relevance (`recency_weight: 0` restores pure fused ranking). Results filter to Memory+Entity labels by default (labels overrides); `kinds` filters to the named memory kinds (`episodic`/`semantic`/`procedural`/`decision`; an unstamped node — entities included — counts as `semantic`, so combine with `labels: ["Memory"]` for memories only; `kinds: ["decision"]` is precedent retrieval — "what did we decide about X?"); text_weight/vector_weight/graph_weight and access_weight tune ranking. `created_after`/`created_before` filter by creation time (after inclusive, before exclusive, applied before top-k); with neither set, `temporal_rewrite` (default true) converts a date phrase in the query ("decisions before 2026-08") into the same filter and searches the residual text — the result then carries `applied_time_filter { after?, before?, source: "params"\|"rewrite", matched_phrase? }`. Returns up to `k` nodes ranked with scores. |
...
| `find_duplicate_memories` | `min_similarity` (0-1, default 0.68), `limit` (1-1000, default 100), `scope?`, `scopes?` | Advisory maintenance scan for near-duplicate memory pairs, most-similar first. Each pair carries a `method` (`"vector"` when embedder is Ready, `"text"` otherwise), a `band` (`likely`/`possible`, in both modes), and a `relation` (`duplicate` → merge / `supersession` → contradicts, retire the stale side). **Vector mode:** cosine ≥ 0.80 `likely` / 0.68-0.80 `possible`; negation-cue check distinguishes contradictions from restatements. **Text mode:** exhaustive pairwise token-containment scoring over the scope (the write-time advisory is the BM25-candidate-limited one — the scan backstops its misses) with fixed 0.7 containment floor; `min_similarity` ignored; band/relation ARE present (lexical-heuristic; bands reuse the cosine cutoffs applied to containment — rough). |
...
| `memory_health` | `stale_older_than_days` (≥ 0, default 30), `scope?`, `scopes?` | One call that runs all three maintenance scans and returns a consolidated summary: `duplicate_pairs` vs `supersession_pairs`, `orphan_count`, `stale_count`, a `needs_attention` flag, and sample rows. The session-start "what needs tidying?" read. Text mode reports the lexical duplicate/supersession split (negation-cue heuristic, same as vector mode). When the embedder is Ready, the split is vector-sourced (contradictions distinguished by cosine + negation cues). When the embedder is Failed or Downloading (degraded), text-mode token-containment scoring applies and `needs_attention` is forced true (even if counts are low), with the state reported in `degraded`/`degraded_reason`. When the embedder is deliberately off, the same text-mode scan still runs and the split counts are real — only `degraded` stays false. |
| `lifecycle_candidates` | `limit` (>= 1, default 20), `half_life_episodic_days`, `half_life_semantic_days`, `half_life_procedural_days` (each > 0; defaults 14/120/365), `now_ms?`, `scope?`, `scopes?` | The Phase C decay sweep: rank live memories by kind-aware staleness (`(age/half_life)/ln(e+access_count)`), top-N with full evidence rows. A `decision`-kind memory shares the semantic half-life (decisions age like standing facts). Read-only and unbumped; deterministic under an injected `now_ms`. |
...
| `get_edges` | `from_id` (required; or target when `direction: "in"`), `to_id?`, `edge_type?`, `direction?` (enum: `"out"`/`"in"`/`"both"`, default `"out"`; for `"in"`, the anchor shifts to the target and `to_id` filters the far source end; `"both"` returns an id-deduped union), `open_only?` (bool, default true when `as_of` absent; omit when passing `as_of`), `as_of?` (integer ms, optional — mutually exclusive with `open_only`), `valid_during?`/`valid_overlaps?` (integer pair — ms range `[from, until]`), `valid_before?`/`valid_after?` (integer ms), `time_axis?` (enum: `"valid"`/`"recorded"`, default `"valid"`), `scope?`, `scopes?` | List a node's outgoing edges, optionally filtered by target node and/or edge type; open edges only by default. `as_of` performs a temporal read within the window `valid_from <= t < valid_to` (valid_to exclusive); omit `open_only` when passing `as_of` (already means "open at that instant"). A future `as_of` behaves like "now". The four `valid_*` interval predicates (`valid_during`, `valid_overlaps`, `valid_before`, `valid_after`) replace the temporal gate — at most one may be set, and each is mutually exclusive with `as_of`, `open_only`, and `time_axis: "recorded"`. This is how a client finds the edge id to `close_edge` when a fact stops being true, and how it checks what a node is already linked to. An `edge_type` filter matches both the normalized and raw stored forms. |
| `traverse` | `seed_id` OR `seed_ids` (non-empty array, wins over `seed_id`), `max_hops` (integer, default 2), `direction` (enum: `out`/`in`/`both`, default `both`), `edge_types` (array of strings, optional), `as_of` (integer ms, optional), `valid_during?`/`valid_overlaps?` (integer pair — ms range `[from, until]`), `valid_before?`/`valid_after?` (integer ms), `time_axis?` (enum: `"valid"`/`"recorded"`, default `"valid"`), `scope?`, `scopes?` | Walk the graph outward from one or more seed nodes, following edges up to `max_hops`. Seed from several nodes at once (e.g. every `search_memories` hit) to explore around all of them in one call. `as_of` performs a temporal read at the given Unix millisecond timestamp (closed edges reappear, later edges vanish); omit for "now". The four `valid_*` interval predicates (`valid_during`, `valid_overlaps`, `valid_before`, `valid_after`) replace the temporal gate — at most one may be set, and each is mutually exclusive with `as_of` and `time_axis: "recorded"`. Returns the subgraph (nodes + edges). |
...
| `graph_snapshot` | `seeds?`, `query?`, `format?` (`json`/`dot`/`mermaid`/`html`/`auto`), `out?`, `max_hops?`, `scope?`, `scopes?` | Export a renderable snapshot: ego view from seed ids and/or a query, or whole-scope when neither is given. `html` writes a self-contained interactive file to `out`. Deterministic; truncation is always reported. Same surface as `topodb graph`. |
| `remember` | `content` (string, required), `entities` (non-empty string array, required), `edge_type` (string, default `"about"`), `kind` (enum: `episodic`\|`semantic`\|`procedural`\|`decision`, optional — omitted reads as `semantic`), `supersedes` (array of memory ids, optional), `check_conflicts` (bool, default true), `props` (object, optional), `scope?` | Store a linked fact in ONE call: creates the memory, find-or-creates each named entity (`create_entity` semantics — case/whitespace-insensitive, alias-aware, never duplicates; repeated names in one call collapse), and links memory→entity — atomically, in a single write batch. Re-storing identical content resolves to the existing memory (`deduplicated: true`). `kind` stamps NEW memories with a taxonomy value (`decision` = a choice made, with its rationale in the content — retrieve precedent later with `search_memories` + `kinds: ["decision"]`); dedup ignores it — the stored kind wins. `supersedes` retires named memories (marks `superseded_at`, closes their open edges) when a fact changes. Returns advisory `near_duplicates` (banded, relation-labeled) and the leaner classified `supersession_candidates` (`{memory_id, relation: duplicate\|supersession, score}`, self-superseded ids excluded) for the just-stored content; `check_conflicts: false` skips the probe and both fields. The preferred storage verb; the tools below are its building blocks. |
