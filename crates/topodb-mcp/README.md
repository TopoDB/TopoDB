...
| `traverse` | `seed_id` OR `seed_ids` (non-empty array, wins over `seed_id`), `max_hops` (integer, default 2), `direction` (enum: `out`/`in`/`both`, default `both`), `edge_types` (array of strings, optional), `scope?`, `scopes?` | Walk the graph outward from one or more seed nodes, following edges up to `max_hops`. Seed from several nodes at once (e.g. every `search_memories` hit) to explore around all of them in one call. Returns the subgraph (nodes + edges). |
