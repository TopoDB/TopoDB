...
`tools/list` reports exactly 17 tools: `db_info`, seven read tools (`get_changes` included), and
nine write tools.
...
| `find_by_prop` | `label`, `prop`, `value` (string/number/bool), `exact` (bool, default false), `scope?`, `scopes?` | Lookup on an equality-indexed property (e.g. an Entity's name). String values match case- and whitespace-insensitively by default; `exact: true` requires a byte-exact match. Call this to resolve a known identifier to a node — for topic/phrase search use `search_memories`. Errors if `(label, prop)` is not declared in the index spec. |
| `search_memories` | `query` (required), `k` (integer, default 10), `recency_weight` (0-1, default 0.3), `recency_half_life_days` (default 30), `fuzzy` (bool, default true), `scope?`, `scopes?` | Full-text BM25 search over indexed text (memory content AND entity names under the default spec), recency-weighted: each hit's score is multiplied by `(1-w) + w·2^(-age/half_life)`, so fresher memories outrank stale ones at equal relevance (`recency_weight: 0` restores pure BM25). Terms are analyzed — camelCase split, lowercased, Snowball-stemmed (`databases` matches `database`) — and a term that matches nothing in a scope falls back to its closest prefix/edit-distance vocabulary neighbors at a 0.6× discount (`fuzzy: false` disables). Synonyms are not understood. Returns up to `k` nodes ranked with scores. |
