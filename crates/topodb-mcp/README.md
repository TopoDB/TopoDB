...
| `search_memories` | `query` (required), `k` (integer, default 10), `recency_weight` (0-1, default 0.3), `recency_half_life_days` (default 30), `scope?`, `scopes?` | Full-text BM25 search over indexed text (memory content AND entity names under the default spec), recency-weighted: each hit's score is multiplied by `(1-w) + w·2^(-age/half_life)`, so fresher memories outrank stale ones at equal relevance (`recency_weight: 0` restores pure BM25). Matching is exact-token after lowercasing — no stemming or fuzzy matching. Returns up to `k` nodes ranked with scores. |
