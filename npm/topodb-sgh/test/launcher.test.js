...
test('binaryFileName is .exe only on win32', () => {
  assert.equal(binaryFileName('win32'), 'sgh.exe');
  assert.equal(binaryFileName('linux'), 'sgh');
  assert.equal(binaryFileName('darwin'), 'sgh');
});

test('subPackageName scopes under @topodb', () => {
  assert.equal(subPackageName('linux-x64'), '@topodb/topodb-sgh-linux-x64');
});
...
test('resolveBinary requests the correct specifier and returns the resolved path', () => {
  ...
  assert.equal(calls[0], '@topodb/topodb-sgh-linux-x64/sgh');
  assert.equal(p, '/abs/@topodb/topodb-sgh-linux-x64/sgh');

  const w = resolveBinary('win32', 'x64', fakeResolve, ok);
  assert.equal(calls[1], '@topodb/topodb-sgh-win32-x64/sgh.exe');
  assert.equal(w, '/abs/@topodb/topodb-sgh-win32-x64/sgh.exe');
});
...
test('resolveBinary gives an actionable error when the platform package is not installed', () => {
  ...
  assert.throws(
    ...
    /prebuilt binary package @topodb\/topodb-sgh-linux-x64 is not installed/,
  );
});
...
test('resolveBinary accepts the platform package when its version matches the launcher', () => {
  ...
  assert.equal(
    resolveBinary('win32', 'x64', resolve, { ownVersion: '0.0.7', versionOf }),
    '/ok/node_modules/@topodb/topodb-sgh-win32-x64/sgh.exe',
  );
});
