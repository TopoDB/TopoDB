...
function subPackageName(key) {
  return `@topodb/topodb-sgh-${key}`;
}

function binaryFileName(platform) {
  return platform === 'win32' ? 'sgh.exe' : 'sgh';
}
...
function resolveBinary(platform, arch, resolve = require.resolve, deps = {}) {
  ...
  if (!key) {
    throw new Error(
      `sgh: unsupported platform ${platform}/${arch}. ` +
        `Supported: darwin-arm64, darwin-x64, linux-x64, linux-arm64, win32-x64. ` +
        `Install from source instead: cargo install topodb-sgh`,
    );
  }
  ...
  try {
    ...
    if (err && err.code === 'MODULE_NOT_FOUND') {
      throw new Error(
        `sgh: the prebuilt binary package ${subPkg} is not installed. ` +
          ...
          `Reinstall the package, or build from source: cargo install topodb-sgh`,
      );
    }
    throw err;
  }
  ...
  try {
    ...
    throw new Error(
      `sgh: resolved a binary at ${binPath} but could not read ${subPkg}'s package.json to ` +
        `confirm it belongs to this install (expected version ${ownVersion}). Refusing to launch an ` +
        `unverifiable binary. Reinstall @topodb/topodb-sgh, or build from source: cargo install topodb-sgh`,
    );
  }

  if (found !== ownVersion) {
    throw new Error(
      `sgh: refusing to launch a foreign binary. Resolved ${subPkg} version ${found} at ` +
        ...
        `above, or reinstall @topodb/topodb-sgh so its own ${subPkg}@${ownVersion} is present.`,
    );
  }
