...
the code — every claim below cites the `storage.rs`/`fts.rs` lines it was read from — and
is mechanically checked against a committed fixture (`crates/topodb/tests/fixtures/v1.redb`,
exercised by `crates/topodb/tests/format_fixture.rs`).
...
```rust
pub const FORMAT_VERSION: u32 = 1;               // storage.rs:34
```
...
(`storage.rs:59-71`). This is the complete table set — there are no others.
...
```rust
pub(crate) const OPS: TableDefinition<u64, &[u8]> = TableDefinition::new("ops");             // storage.rs:12
pub(crate) const META: TableDefinition<&str, &[u8]> = TableDefinition::new("meta");           // storage.rs:13
pub(crate) const NODES: TableDefinition<&[u8], &[u8]> = TableDefinition::new("nodes");        // storage.rs:14
pub(crate) const EDGES: TableDefinition<&[u8], &[u8]> = TableDefinition::new("edges");        // storage.rs:15
pub(crate) const POSTINGS: TableDefinition<&[u8], &[u8]> = TableDefinition::new("postings");  // storage.rs:19
pub(crate) const FTS_DOCS: TableDefinition<&[u8], &[u8]> = TableDefinition::new("fts_docs");  // storage.rs:22
pub(crate) const FTS_STATS: TableDefinition<&[u8], &[u8]> = TableDefinition::new("fts_stats");// storage.rs:28
pub(crate) const COUNTERS: TableDefinition<&[u8], &[u8]> = TableDefinition::new("counters");  // storage.rs:32
```
...
- **`node_key(id: NodeId) -> [u8; 16]`**: the node's ULID as 16 big-endian bytes
  (`id.0.0.to_be_bytes()`) — `storage.rs:665-667`. Big-endian keys make redb's natural
...
- **`scope_key(s: Scope) -> [u8; 17]`**: a fixed-width 17-byte encoding — 1-byte tag
  ...
  (all-zero for `Shared`) — `storage.rs:700-718`:
  ...
  be a prefix of another scope's 17-byte prefix (`storage.rs:700-704`).
- **Postings key**: `posting_key(scope, term) = scope_key(scope) ++ term.as_bytes()`
  (UTF-8 term bytes, unescaped) — `fts.rs:124-130`.

### `ops` table (value: `Op`)

```rust
pub enum Op {                                                    // op.rs:11-27
...
time, so a stored op never depends on "now" and replay is deterministic (`op.rs:7-9`,
`storage.rs:647-663`).
...
|---|---|---|---|
| `"format_version"` | `u32`, little-endian, 4 bytes (`to_le_bytes`) | `Storage::open_with` on first create (`storage.rs:86-89`) | never absent after first open |
| `"index_spec"` | postcard-encoded `IndexSpec`, **both `equality` and `text` sorted by `(label, prop)`** | `Storage::ensure_index_spec`, every open (`storage.rs:185-197`) | pre-index-spec (Plan-1) file |
| `"oldest_seq"` | `u64`, little-endian, 8 bytes (`to_le_bytes`) | `Storage::compact_ops_through` (`storage.rs:313-315`) | log never compacted — treat as `1` |

`"index_spec"`'s sort is what makes the persisted value canonical: reordering the same
declarations in code never looks like a spec change (`storage.rs:688-698`, doc comment at
`storage.rs:113-121`). Only the **text** portion drives a reindex when it changes; the
equality index is rebuilt in memory from `NODES` on every open, so it is persisted purely
for introspection/tooling (`storage.rs:118-121`).
...
per-scope layout, and then deleted (`storage.rs:124-133,145-146,187-190`). No v1-produced
...
```rust
pub struct NodeRecord {                                    // state.rs:6-13
    pub id: NodeId,
    pub scope: Scope,
    pub label: SmolStr,
...
```rust
pub struct EdgeRecord {                                     // state.rs:15-25
    pub id: EdgeId,
    pub scope: Scope,
    pub ty: SmolStr,
    pub from: NodeId,
...
the on-disk order is deterministic (`fts.rs:86-118`). A node with term-frequency 0 for a
term is removed from the list; an empty list removes the row entirely (`fts.rs:104-117`).

### `fts_docs` table (value: `u32`)

One row per node that currently has non-empty declared text: its token count. Removed
when the node's declared text becomes empty (`fts.rs:166-178,257-263`).
...
(`fts.rs:132-164`). Keying this per scope (rather than one global pair, as in the
superseded Plan-2 `"fts_doc_count"`/`"fts_total_len"` META counters) is what makes BM25
scores scope-isolated: indexing a document in scope B never shifts scope A's `df`/`avgdl`
(`storage.rs:23-27`, `fts.rs:1-15`). Row dropped when a scope's last document is removed
(`fts.rs:149-164`).

### `counters` table (value: `AccessStats`)

```rust
pub struct AccessStats {                                    // counters.rs:19-23
...
(`storage.rs:29-31,566-568`; module doc at `counters.rs:1-12`). A bump is folded
read-modify-write by the applier thread from a best-effort, fire-and-forget channel
(`storage.rs:504-532`, `db.rs:262-271`). The practical consequence: **deleting (or losing)
the `counters` table loses only access statistics** — recency/hit-count telemetry a host
may use for decay heuristics — never graph state, and never anything replayable from
`OPS`. A `RemoveNode` may even leave an orphaned counter row behind for a since-deleted
node; this is considered benign, since reads of stats gate on current node existence
(`counters.rs:10-12`).
...
- **`oldest_seq`** (`Storage::oldest_seq`, `storage.rs:255-263`): the oldest op sequence
  number still retained. Sourced from `META["oldest_seq"]`; absent means the log has
  never been compacted, so the floor is `1`.
- **`Storage::compact_ops_through(keep_from)`** (`storage.rs:282-319`) drops every `OPS`
  entry with seq `< keep_from` and stamps the new floor into `META["oldest_seq"]`, in one
  write transaction. `keep_from <= oldest_seq` is a no-op; `keep_from > current_seq + 1`
  is rejected (`TopoError::Rejected`) as skipping never-written seqs; `keep_from ==
  current_seq + 1` is legal and empties the log entirely.
- **`Compacted { oldest }`** (`error.rs:12-13`) is returned by two call sites once the
  requested range dips below the retained floor:
  - `Storage::read_ops(since)` (backing `Db::ops_since`) when `since < oldest_seq`
    (`storage.rs:320-348`) — the caller must re-anchor from materialized state rather
    than receive a silently partial replay.
  - `Storage::rebuild_state_from_ops` (backing the `#[doc(hidden)]`
    `Db::rebuild_state_from_ops`) refuses outright once `oldest_seq > 1`
    (`storage.rs:570-579`): after compaction the log is no longer a full history, so a
    from-genesis replay is impossible by definition, and `NODES`/`EDGES` remain the
    source of truth with no full-history fallback.
- **Floor clamp at both append sites.** Emptying the log via compaction
  ...
  `Storage::append_ops` (`storage.rs:230-241`) and `Storage::apply_batch`
  (`storage.rs:417-435`) — read that floor **inside the same write transaction** as the
  append and clamp `next_seq` up to it (`.max(floor)`), which is what keeps seqs
...
(`storage.rs:34`, checked at `storage.rs:75-103`). The policy for changing it:

- **Minor, no version bump**: adding a new table, or a new `META` key, that older
  readers simply never open/read. (`Storage::open_with` unconditionally opens all eight
  current tables; a reader from a version that predates a new table would need its own
  code change to look at it, but does not choke on its *presence* — redb tables are
  opened by name on demand, not enumerated.) Adding a new `Op` variant that only newer
  code emits falls in the same bucket, since `Op` decoding is postcard/serde-driven per
  build.
- **Format-version bump required**: any change to the **encoding** of an existing
  table's key or value, or of an existing `META` key's value (e.g. changing `node_key`
  from big-endian to little-endian, changing `postings`' value from a `Vec` to a `HashMap`,
  changing `AccessStats`'s field types) — anything that would make an old build
  misinterpret bytes written by a new build, or vice versa. `FORMAT_VERSION` must be
  bumped and `Storage::open_with`'s version-compat handling (`storage.rs:85-103`)
  extended if any migration path is intended; the current code only accepts an
  exact-match version and rejects anything newer (`UnsupportedFormat`).
